...
const kIsStraw = Symbol('kIsStraw');
const kIsReclaimer = Symbol('kIsReclaimer');
...
function createPausedSocket(pipe) {
  return net.Socket({
    handle: pipe,
    readable: true,
    pauseOnCreate: true,
...
ChildProcess.prototype.spawn = function spawn(options) {
  ...
  for (i = 0; i < stdio.length; i++) {
    ...
    if (stream.handle) {
      ...
      if (stream.reclaimer !== undefined) {
        stream.socket[kIsStraw] = true;
        const reclaimer = createPausedSocket(stream.reclaimer);
        reclaimer[kIsReclaimer] = true;
        stream.socket.reclaimer = reclaimer;
        reclaimer.on('close', getMaybeClose(this));
      }

      if (i > 0 && this.pid !== 0)
        stream.socket.on('close', getMaybeClose(this));
    }
  }
  ...
  stdio = ArrayPrototypeReduce(stdio, (acc, stdio, i) => {
    function cleanup() {
      for (let i = 0; i < acc.length; i++) {
        if ((acc[i].type === 'pipe' ||
             acc[i].type === 'ipc' ||
             acc[i].type === 'straw') && acc[i].handle) {
          acc[i].handle.close();
          acc[i].reclaimer?.close();
        }
      }
    }
    ...
    if (stdio === 'ignore') {
      ...
      a.handle = new Pipe(PipeConstants.STRAW);
      a.reclaimer = new Pipe(PipeConstants.SOCKET);
      ...
      if (stdio[kIsStraw] || stdio[kIsReclaimer]) {
        cleanup();
        throw new ERR_INVALID_ARG_VALUE('stdio', stdio);
      }
...
function getMaybeClose(subprocess) {
  let closed = false;
  subprocess._closesNeeded++;
  return function() {
    assert(!closed, 'close callback called multiple times');
    closed = true;
    maybeClose(subprocess);
  };
}
